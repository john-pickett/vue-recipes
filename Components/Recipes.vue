        ...
        <v-container grid-list-md>
            <v-layout row wrap>
                <v-flex md6 v-for="(recipe, key) in recipes">
                    <draggable :id="key" v-model="recipes"
                               :options="{group: { name:'recipes', pull:'clone', put:'false'}}" @start="drag=true"
    ...
    export default {
        ...
        methods: {
            ...
            sendRecipeData: function (id) {
                const myRecipe = this.recipes.filter(function (recipe) {
            ...
            inputRecipe: function () {
                const elem = document.getElementById('recipe-input');
                const recipeId = this.recipes.length;
